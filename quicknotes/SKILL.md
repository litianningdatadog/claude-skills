...
Notes are one markdown file per note under `~/.claude/btw/notes/<id>.md` (override with
...
from disk** (a hard delete — there is no cancelled/archived state). The `~/.claude/btw` store
